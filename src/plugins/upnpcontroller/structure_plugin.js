...
define( ['structure_custom', 'css!plugins/upnpcontroller/upnpcontroller' ], function( VisuDesign_Custom ) {
  ...
  function refreshUpnpcontroller(path, controller, d, oneTimeCall) {
    var upnpcontroller = $(controller);
    ...
    function updateContoller( volume, mute, playMode, title, reltime, duration, artist, album){
      ...
      var mute = $('#' + upnpcontroller_uid + '_muteButton');
      var play = $('#' + upnpcontroller_uid + '_playButton')
      if(mute == 0){
        mute.removeClass('switchPressed');
        mute.addClass('switchUnpressed');
      }else{
        mute.removeClass('switchUnpressed');
        mute.addClass('switchPressed');
      }

      if(playMode == 'Play'){
        play.removeClass('switchUnpressed');
        play.addClass('switchPressed');
      }else{
        play.removeClass('switchPressed');
        play.addClass('switchUnpressed');
      }
